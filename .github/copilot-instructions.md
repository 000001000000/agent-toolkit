...
1. Treat `skills/dev-playbook/SKILL.md` as mandatory first-pass guidance for:
- Frontend work
- Backend work
- Full-stack work
- Refactoring
...
- Before executing TDD guidance, load and follow `.agents/skills/tdd/SKILL.md` as the canonical TDD workflow.
- Red: write or update a failing test.
- Green: implement the smallest change to pass.
- Refactor: clean up while tests remain green.

3. For frontend work, prioritize reusable component architecture and visual verification.
4. Enforce reuse-first implementation for all code, with extra emphasis on frontend:
...
5. For backend work, prioritize contract-first interfaces, validation, reliability, and security baseline practices.

6. Apply backward compatibility conditionally:
...
7. Always report verification steps performed (tests/lint/build/visual checks) and any gaps.
8. Include this decision checklist in implementation summaries:
...
9. Enforce a Definition of Done in implementation summaries:
...
10. No hidden complexity and ask-before-overengineering:
