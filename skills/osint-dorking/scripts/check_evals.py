...
def print_scorecard(cases: Iterable[EvalCase], results: Dict[int, EvalResult], color: bool) -> int:
    ...
    if trig_total:
        print(f"- Trigger recall: {trig_pass}/{trig_total} ({(trig_pass / trig_total) * 100.0:.1f}%)")
    if no_trig_total:
        print(
            "- Non-trigger precision: "
            f"{no_trig_pass}/{no_trig_total} ({(no_trig_pass / no_trig_total) * 100.0:.1f}%)"
        )

    # Non-zero exit on failure or missing outcomes so CI can catch regressions.
    return 1 if counts["FAIL"] > 0 or counts["MISSING"] > 0 else 0
...
def main() -> int:
    ...
    return print_scorecard(cases, results, color_enabled)
